/* ქართული — тренажёр грузинского. Данные: data/*.json, озвучка: audio/<voice>/<hash>.mp3 */
'use strict';

const LS_KEY = 'kartuli_progress_v1';
  ...
  const promptHTML = mode === 'listen'
    ...
    : askKa
      ? `<div class="word-card"><span class="lvl">${w.lvl}</span>
           ...
           <div class="word-tr">выберите перевод на грузинском</div></div>`
      : wordCardHTML(w, {});
  const box = el(`<div class="trainer">
    ${trainerHead(o)}
    ${promptHTML}
...
function exerciseRecall(w, o) {
  ...
  const front = backwards
    ? `<div class="word-ka" style="font-size:30px">${esc(w.ru)}</div>
       <div class="word-tr">вспомните слово по-грузински</div>`
    : `<div class="word-ka ka">${esc(w.ka)}</div>
       ${S.prog.set.translit ? `<div class="word-tr">${esc(w.tr)}</div>` : ''}`;
  const box = el(`<div class="trainer">
    ${trainerHead(o)}
...
function exerciseTyping(w, o) {
  const box = el(`<div class="trainer">
    ...
    <div class="word-card">
      ...
      <div class="word-tr">напишите по-грузински — или латиницей, как слышится</div>
      <button class="speak lg" title="Произношение">🔊</button>
      <div class="typing">
    ...
    <p class="hint">Принимается и грузинское написание, и латиница · <kbd>Enter</kbd> — проверить</p>
  </div>`);
  bindSpeak(box, w.ka);
...
function openSettings() {
  ...
  const bg = el(`<div class="modal-bg"><div class="modal">
    ...
    <div class="set-group">
      ...
      ${row('Показывать транслитерацию', 'Латинская запись под грузинским словом', `<span data-sw="translit">${sw(st.translit)}</span>`)}
    </div>
    ...
    <div class="set-group">
      ${row('Голос', 'Microsoft Neural, грузинский (ka-GE)', `<select id="s-voice">
        <option value="f" ${st.voice === 'f' ? 'selected' : ''}>Эка (женский)</option>
        <option value="m" ${st.voice === 'm' ? 'selected' : ''}>Гиорги (мужской)</option></select>`)}
  ...
  speed.onchange = () => { saveProgress(); speak('გამარჯობა'); };

  $('#s-check', bg).onclick = async () => {
    const word = 'გამარჯობა', lines = [];
...
/* ---------------- оформление ---------------- */
function themePref() { return localStorage.getItem('kartuli_theme') || 'system'; }
function applyTheme() {
...
async function boot() {
  ...
  mq.addEventListener('change', () => {
    if ((localStorage.getItem('kartuli_theme') || 'system') === 'system') { applyTheme(); render(); }
  });
  try {
    const [wd, al, ai, mn] = await Promise.all([
      ...
      fetch('data/mnemonics.json').then(r => r.json()).catch(() => ({})),
    ]);
